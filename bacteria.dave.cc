...
public:
	EvoBacterium()
	{
		...
		SetReproductionThreshold(randval(1.0));	// default: 0.4
		SetConsumptionRate(randval(2.0));		// default: 0.1
		...
		SetNutrientResponse(0.0);//randval(2.0));		// default: 0.8
		...
		SetSpeed(randval(30.0));				// default: 40.0
...
//		SetRadius(					g[14]);
	...
	virtual float GetFitness()const
	{
		...
		Vector2D worldCentre(GetWorld().GetWidth() / 2.0, GetWorld().GetHeight() / 2.0);
		float avDist = 0.0;
		float topDist = 0.0;

		for (list<Bacterium const*>::const_iterator i = babies.begin(); i != babies.end(); ++i) {
			avDist += ((*i)->GetLocation() - worldCentre).GetLength();
			topDist = MAX(topDist, ((*i)->GetLocation() - worldCentre).GetLength());
		}
...
class ChiralBacterium : public EvoBacterium
{
public:
	ChiralBacterium()
	{
	...
	*/
	virtual void Update()
	{
		...
		vel.SetLength(GetMaxSpeed());

		SetVelocity(vel);

		UpdateEnergy();

		vel.SetAngle(GetVelocity().GetAngle() + chiral);
	...
	void	SetChiral(double c) {chiral=c;while(c>PI)c-=PI;while(c<-PI)c+=PI;}
	double	GetChiral()const	{ return chiral; }
/*
	virtual genotype_type GetGenotype()const
	{
		genotype_type g = EvoBacterium::GetGenotype();
		g.push_back(GetChiral());
		return g;
	}

	virtual void SetGenotype(const genotype_type& g)
	{
		EvoBacterium::SetGenotype(g);
		SetChiral(g[15]);
	}
*/
	virtual void OnClick()
	{
		EvoBacterium::OnClick();
		GetLogStream() << "Chiral: " << GetChiral() << std::endl;
	}
...
public:
	...
	grpBacteria(30, gaBacteria)
	{
		...
		GetWorld().SetWidth(800.0);	// default: 800.0
		GetWorld().SetHeight(600.0);	// default: 600.0
		...
		// details.
		gaBacteria.SetSelection(GA_RANK);
		...
		nutrient = new Distribution(400, 300, 2);
		nutrient->SetColour(1.0f, 1.0f, 1.0f);
		// The diffusion kernel works on the distribution every frame. Set
		// diffusion speed to 2 for every other frame, 0 for never.
		nutrient->SetDiffusionSpeed(1);
		grpWorld.push_back(nutrient);	// Add the distribution to grpWorld.
...
//		every other frame.
		...
		attractant->SetColour(0.3f, 1.0f, 0.3f);
...
//		every frame.
		...
		repellent->SetDiffusionSpeed(1);
		repellent->SetColour(0.5f, 1.0f, 0.5f);
...
//*/
		...
		trail->SetColour(0.9f, 0.9f, 1.0f);
		grpWorld.push_back(trail);

		Add("Bacteria", grpBacteria);
		Add("Distributions", grpWorld);
		...
		SetTimeSteps(200);	// default: 1000
//		SetAssessments(30);	// default: 1 - don't uncomment this here,
					// use the other simulation object instead
		SetGenerations(2000);	// default: unlimited
//		SetRuns(1);		// default: 1
	...
	virtual void BeginAssessment()
	{
		...
		nutrient->Plot(0.35);
//		nutrient->Plot(Gaussian2D(200, 150, 30.0, 3000.0));
//		nutrient->Filter(plus<double>(), GaussianRing2D(200, 150, 100.0, 5.0, 10.0));
		nutrient->Filter(plus<double>(), GaussianNoise(0.0, 0.5));
//		nutrient->Filter(bind2nd(divides<double>(), 1.5));
		// This can be used to clip the distribution within certain values.
//		nutrient->Filter(LimitDistribution(0.0, 3.0));
		...
		nutrient->SetMaxConc(0.6);

		trail->Plot(0.0);	// Reset the trail distribution.
		attractant->Plot(0.0);	// Reset the attractant distribution.
		repellent->Plot(0.0);	// Reset the repellent distribution.
	...
	{
		ostringstream fname;
		fname << "bacteria" << GetGeneration() << ".jpg";

//		ScreenGrab(GetWorld(), fname.str());
//		ScreenGrab(GetWorld(), "bacteria.jpg");

		Simulation::EndAssessment();
	}
...
public:
	BacteriaFractalSim2()
	{
		grpBacteria.SetTeamSize(1);	// One individual per assessment.
		SetAssessments(30);		// One assessment per grpBacteria member.
	}
};
